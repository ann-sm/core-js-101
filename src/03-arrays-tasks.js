...
function getMovingSum(arr) {
  return arr.map((x, i) => x + arr.slice(0, i).reduce((acc, el) => acc + el, 0));
...
function propagateItemsByPositionIndex(arr) {
  return arr.flatMap((x, index) => Array.from({ length: index + 1 }).fill(x));
  // throw new Error('Not implemented');
}
...
function sortCitiesArray(arr) {
  ...
  // return arr.sort((x, y) => x.city > y.city).sort((a, b) => a.country > b.country);
  // throw new Error('Not implemented');
}
...
function getIdentityMatrix(n) {
  return Array.from({ length: n }, (x, i) => new Array(n).fill(0).map((y, j) => (i === j ? 1 : 0)));
...
function getIntervalArray(start, end) {
  return Array.from({ length: end - start + 1 }, (x, i) => start + 1 * i);
  // throw new Error('Not implemented');
}
...
function group(/* array, keySelector, valueSelector */) {
  // return array.map((x,) => keySelector);
  throw new Error('Not implemented');
}
...
function getElementByIndexes(/* arr, indexes */) {
  // return indexes.reduce((x, i) => arr[i][i]);
  throw new Error('Not implemented');
}
